...
for a in range(1):
    ...
    def resource_allocation(p, T, time):
        ...
        #model += np.sum([X[j, b] * geodesic(C[LW[cluster_labels[b]]], C[b]).miles for j in range(warehouse) for b in range(z)])/den_economic
        ...
        # Calinski-Harabasz Histogram
        '''
        x_var_ch = []
        y_var_ch = []
        for alpha in range(3, 11):
            x_var_ch.append(alpha)
            kmeans_test = KMeans(n_clusters=alpha, random_state=0).fit(C)
            y_var_ch.append(calinski_harabasz_score(C, kmeans_test.labels_))

        plt.bar(x_var_ch, y_var_ch, edgecolor = 'black')
        plt.ylabel('Calinski-Harabasz Score', fontsize=14)
        plt.xlabel('Number of Clusters', fontsize=14)
        plt.tight_layout()
        plt.savefig('calinski_harabaszt_score_547.png', dpi=300)
        plt.show()
        '''

        # Cluster + Warehouse Plot
        '''
        plt.grid(linestyle='--')
        colors = ['blue', 'red', 'green', 'orange', 'purple']
        for chi in range(warehouse):
            plt.scatter([C[label_arr[chi][pi]][0] for pi in range(len(label_arr[chi]))], [C[label_arr[chi][pi]][1] for pi in range(len(label_arr[chi]))], color=colors[chi], s=20, alpha = 0.75)
        plt.scatter([C[LW[omega]][0] for omega in range(warehouse)], [C[LW[omega]][1] for omega in range(warehouse)], marker="x", color='black', s=90.0)
        ...
        plt.tight_layout()
        plt.savefig('cluster_map_with_warehouses_537.png', dpi=300)
        plt.show()
        '''
