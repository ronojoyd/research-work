                    ...
                    for delta in range(z):
                        ...
                        for phi in range(len(arr)):
                            while(available_vaccine > 0):
                                ...
                                entities[arr[phi]].state = np.random.choice([initial_state, 'R'], size=1, p=[0.8, 0.2])[0]
...
def resource_allocation(T=vaccines, time=T, p=0.3, fl=0.05, fh=0.2):
    ...
    #std = (np.std(array, axis=1)).reshape(z, ) # st. dev of distances from warehouses
    ...
    r = np.array([0.2 for y in range(z)])  # potency of the vaccine
