...
for alphabetagammaepsilon in range(1): 
    ...
    def resource_allocation(p, T, time):
        ...
        Z = [((B[i]-np.mean(B))/np.sum(B))+1.0 for i in range(z)]
        ...
        plt.plot([0, 14], [intercept, (slope*14 + intercept)], color='gray', linestyle='--'])
        
        plt.scatter(vaccines_per_zone, another_arr, s=5)
        plt.ylabel('Susceptible and Population Density Value', fontsize=10)
        plt.xlabel('Vaccines Per Zone', fontsize=14)
        plt.tight_layout()
        plt.savefig('susceptible_and_pop_density.png')
        plt.show()
        
        print('Z', Z)
        
        summm = [B[i] - np.mean(B) for i in range(z)]
        print('\nB[i] - np.mean[B]', np.sum(summm))
